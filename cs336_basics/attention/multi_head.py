...
class MultiHeadAttention(torch.nn.Module):
    def __init__(self, d_model: int, num_heads: int, max_seq_len: int | None = None, theta: float | None = None, device: str = "cpu"):
        ...
        self.d_v = self.d_q
        self.d_o = self.d_q
        self.q_proj = Linear(self.d_model, self.d_model, device=self.device)
        self.k_proj = Linear(self.d_model, self.d_model, device=self.device)
        self.v_proj = Linear(self.d_model, self.d_model, device=self.device)
        self.output_proj = Linear(self.d_model, self.d_model, device=self.device)
        self.rope = None
        if theta:
    ...
    def forward(self, in_features: Float[Tensor, "... seq_len d_model"], token_positions: Int[Tensor, "... seq_len"] | None = None) -> Float[Tensor, "... seq_len d_model"]:
        ...
        q = rearrange(self.q_proj(in_features), "... seq_len (num_head head_dim) -> ... num_head seq_len head_dim", num_head=self.num_heads)
        k = rearrange(self.k_proj(in_features), "... seq_len (num_head head_dim) -> ... num_head seq_len head_dim", num_head=self.num_heads)
        v = rearrange(self.v_proj(in_features), "... seq_len (num_head head_dim) -> ... num_head seq_len head_dim", num_head=self.num_heads)
