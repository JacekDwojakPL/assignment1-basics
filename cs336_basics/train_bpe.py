...
def train_bpe(input_path, vocab_size, special_tokens):
    ...
    merges = []
    word_counts, pair_counts, pair_to_word, words_to_pair = get_word_counts(input_path, 24, "<|endoftext|>".encode())
    
    for i in range(vocab_size):
        new_pair = None
        # get most frequent pair
        most_frequent_pair = sorted(pair_counts.most_common(), key=lambda pair: (pair[1], pair[0]), reverse=True)[0]
